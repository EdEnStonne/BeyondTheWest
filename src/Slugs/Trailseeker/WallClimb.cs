...
public class WallClimbObject
{
    ...
    {
        public WallClimbManager(AbstractCreature abstractCreature)
        {
            this.abstractPlayer = abstractCreature;
            if (Plugin.meadowEnabled)
            {
                MeadowCompat.WallClimbMeadow_Init(this);
            }
        }
        ...
        public Vector2 DirectionalInput
        {
            get
            {
                ...
                if (player != null)
                {
                    Player.InputPackage cinput = player.input[0];
    ...
    private static void Player_WallClimbManager_CancelWallPounce(ILContext il)
    {
        ...
        try
        {
            ...
            {
                static bool CheckWallClimbingPounce(bool orig_bool, Player player)
                {
                    if (TrailseekerFunc.cwtClimb.TryGetValue(player.abstractCreature, out var WCM) && (
                        WCM.rocketJumpFromWallVerticalPounce
                    ))
                    {
                        return false;
        ...
        Plugin.Log("WallClimbManager IL 1 ends");
    }
    ...
    private static void Player_WallClimbManager_StopLoopSoundOnGrip(ILContext il)
    {
        ...
        try
        {
            ...
            ))
            {
                ...
                {
                    if (orig && TrailseekerFunc.cwtClimb.TryGetValue(player.abstractCreature, out var WCM))
                    {
